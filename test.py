...
<style>
    ...
    .streamlit-expanderHeader:hover {
        color: #2c2c2c !important;
        background: linear-gradient(135deg, #faf8f6 0%, #f5f0eb 100%) !important;
    }
    ...
    /* Force all text in expander headers to be large */
    ...
    /* Make each expander more distinct */
    ...
    div[data-testid="stExpander"]:hover {
        box-shadow: 0 8px 20px rgba(109,76,65,0.15) !important;
    }

    /* Different border color for each expander */
    div[data-testid="stExpander"]:nth-child(odd) .streamlit-expanderHeader {
        border-left: 4px solid #6d4c41 !important;
    }

    div[data-testid="stExpander"]:nth-child(even) .streamlit-expanderHeader {
...
def show_main_app():
    ...
    st.markdown('<div class="custom-divider"></div>', unsafe_allow_html=True)  # Divider line
    st.markdown('<div style="height: 1rem;"></div>', unsafe_allow_html=True)  # Extra spacing
        ...
        with st.spinner("☕ Brewing your personalized recommendations..."):
            ...
            if not results:
                ...
                **No coffee purchase history found.**  
                ...
                # Add spacing before success message
                st.markdown('<div style="height: 1rem;"></div>', unsafe_allow_html=True)
                
                # Success message (will be centered by CSS)
                st.success("🎉 Here are your personalized pairings! 🎉")
                
                # Add spacing after success message
                st.markdown('<div style="height: 1rem;"></div>', unsafe_allow_html=True)
                    ...
                    with st.expander(coffee_title, expanded=True):
                        ...
                        with col_rec1:
                            if recs['popularity']:
                                ...
                                # Add top spacing to push card down
                                st.markdown('<div style="height: 2rem;"></div>', unsafe_allow_html=True)
                                
                                # Display the actual recommendation
                        ...
                        with col_rec2:
                            if recs['flavor']:
                                ...
                                # Get flavor notes and create description with badges
                                if flavors:
                                    unique_flavors = list(dict.fromkeys(flavors))
                                    
                                    # Create the flavor badges HTML
                                    badge_html = ""
                                    for f in unique_flavors:
                                        badge_html += f'<span class="flavor-badge" style="background: #e8e0d8; margin-right: 0.4rem;">{f.title()}</span>'
                                    
                                    # Add the description with badges
                                    st.markdown(f"""
                                    <div style="margin-bottom: 1rem;">
